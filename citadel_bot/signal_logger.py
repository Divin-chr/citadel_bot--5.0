    ...
    def log_signal(
        ...
        try:
            ...
            signal_data = {
                ...
                'trend_daily': str(bool(ta_result.trend_daily)).upper(),
                'trend_weekly': str(bool(ta_result.trend_weekly)).upper(),
                'trend_monthly': str(bool(ta_result.trend_monthly)).upper(),
                'rsi': round(ta_result.rsi, 2),
                'macd_hist': round(ta_result.macd_hist, 5),
                'macd_cross': str(bool(ta_result.macd_cross)).upper(),
                ...
                'rt_momentum': round(delta.rt_momentum, 6) if delta else None,
                'delta_aligned': bool(delta.aligned) if delta else None,
                'alignment_score': round(delta.alignment_score, 4) if delta else None,
            ...
            if self._db_available:
                asyncio.create_task(self._log_to_database(signal_data, sym))
