...
class RiskManager:
    ...
    def _check_macro_calendar(self) -> bool:
        ...
        # Warn if calendar is missing (trading without full protection)
        if self._calendar_missing:
            log.warning(
                "[%s] Trading without economic calendar protection. "
                "High-impact events (NFP, FOMC, CPI) will NOT trigger auto-halt.", now.strftime("%Y-%m-%d %H:%M")
            )
    ...
    def _check_position_count(self) -> bool:
        if self._positions_today >= self.config.max_concurrent_positions:
            log.debug("Max concurrent positions (%d) reached.", self.config.max_concurrent_positions)
            return False
        return True
    ...
    def position_opened(self):
        ...
        # v2.3: Track for correlation
        # Note: caller should pass sym, direction, etc. — simplified for now
